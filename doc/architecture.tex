...
\section{Type Systems for Working Mathematician}

\subsection{The Prerequisites}
...
\begin{tabular}{lll}
\hline
\textbf{System} & \textbf{Semantic Category / Model} & \textbf{Application \& Physical/Mathematical Domain} \\
\hline
$O_\lambda$ & Cartesian Closed Category ($\mathbf{CCC}$) & Untyped $\lambda$-calculus (evaluation and interpretation) \\
$O_\pi$ & Symmetric Monoidal Category ($\mathbf{SMC}$) & Process calculi (CCS, CSP, Milner's $\pi$-calculus) \\
$O_\mu$ & Tensor Category ($\mathbf{TC}$) & Tensor calculus (vectorization) \\
\hline
...
\newpage
\subsection{The Three Spines of the Language Lattice}
...
\newpage
\subsection{The Cosmic Cube Classification}
The terminal object of the language lattice, the \textbf{Cosmic Cube}, classifies the various type theories by mapping the three spines to the axes of a three-dimensional coordinate system. The axes represent fundamental semantic properties of the systems:
\begin{enumerate}
    \item \textbf{Strictness Axis ($\Delta \leftrightarrow \nabla$):} Models the distinction between strict (presheaf-like, $\Delta$) and modal/cohesive/directed (sheaf-like, $\nabla$) semantics.
    \item \textbf{Groupoidality Axis ($\mathbf{2} \leftrightarrow \mathbb{N}$):} Distinguishes 1-categorical/set-theoretic models ($\mathbf{2}$) from higher homotopy-theoretic / $\infty$-groupoid models ($\mathbb{N}$).
    \item \textbf{Stability Axis ($\mathbf{1} \leftrightarrow \mathbb{H}$):} Contrasts standard cartesian logic/resource-insensitive type theory ($\mathbf{1}$) with stable/linear/quantum logic and spectra ($\mathbb{H}$).
\end{enumerate}

Specifically, the three spines of the Groupoid Infinity stack correspond to moving along the axes of this Cosmic Cube:
\begin{itemize}
    \item \textbf{Spine I (Set Analysis):} Maps to the \textbf{Strictness Axis ($\Delta \leftrightarrow \nabla$)}, exploring the transitions from strict constructive inductive systems (Frank, Christine) to modal and topological analysis (Laurent).
    \item \textbf{Spine II (Super Homotopy):} Maps to the \textbf{Groupoidality Axis ($\mathbf{2} \leftrightarrow \mathbb{N}$)}, moving from set-theoretic base languages (Per) to univalent homotopy type theory and $\infty$-groupoids (Anders).
    \item \textbf{Spine III (Infinity Category Theory):} Maps to the \textbf{Stability Axis ($\mathbf{1} \leftrightarrow \mathbb{H}$)}, formalizing stable category theory, chain complexes, operational presentations, and stable spectra (Dan, Mike, Ulrik).
\end{itemize}

Under this geometric classification, the vertices of the Cosmic Cube correspond to the following configurations:
\begin{table}[ht!]
\centering
\caption{Vertices of the Cosmic Cube}
\label{tab:cosmic_cube}
\begin{tabular}{ccl}
\hline
\textbf{Configuration} & \textbf{Structure} & \textbf{Description \& Primary System} \\
\hline
$(\Delta, \mathbf{2}, \mathbf{1})$ & $\Pi\Sigma$ (MLTT-75) & Dependently typed $\lambda$-calculus --- Per \\
$(\Delta, \mathbf{2}, \mathbb{H})$ & Linear $\Pi\Sigma$ & $\lambda$-calculus, resource-sensitive computation --- Urs \\
...
$(\nabla, \mathbf{2}, \mathbf{1})$ & Modal MLTT & Modal Dependently Typed $\lambda$-calculus --- Anders \\
$(\nabla, \mathbf{2}, \mathbb{H})$ & $\infty$-toposes & $\infty$-toposes, Quantum Field Theory (dirtt, stt) --- Dan \\
...
\newpage
\subsection{The Semantic Foundations}
...
\begin{itemize}
    ...
    \item \textbf{Homotopical Presheaves:} Bridging the gap from strict 1-categories to full $\infty$-categories, Thierry Coquand's Groupoid-Valued Presheaf and sheaf models ($\mathbf{Grpd}\text{-}\mathbf{PSh}$) \cite{coquand2021constructive} (with higher sheaf extensions \cite{Coq88}) provide a concrete constructive semantics for univalence, where dependent types are interpreted via the \emph{Grothendieck Construction}. This establishes the semantic foundation for Cubical Type Theory.
...
\newpage
\subsection{The Internal Languages}
...
To contextualize the development and syntactic choices of the internal languages in our stack (such as Henk, Per, Anders, Frank, and Christine), Table~\ref{tab:type_theories_comparison} compares the constructor support of historical and contemporary type systems. This comparison maps the evolution of dependent products ($\Pi$), dependent sums ($\Sigma$), identity types ($\text{Id}$), cubical paths ($\Xi$), universe structures ($U^n$), and various approaches to inductive definitions ($\mathbb{N}$, finite types $0/1/2$, $W$-types, general inductive definitions $\mathrm{Ind}$, and general recursion/fixpoints $\mathrm{Fix}$).

% Historical and contemporary comparison of type theories by support for primitive language constructs.
% Originally imported from ./books/vol1/mltt.tex.
\begin{table}[ht!]
  \centering
  \caption{Feature comparison of historical and contemporary type theories.}
  \label{tab:type_theories_comparison}
  \noindent\makebox[\textwidth][c]{%
...
System P$_\omega$ (CoC-88)  &   & x &   &   &   &   &   &   &   &   \\
       ...
       MLTT-75              & x & x & x & x &   & x & x &   &   &   \\
       MLTT-80              & x & x & x & x &   &   &   & x &   &   \\
       Anders (HTS)         & x & x & x & x & x & x & x & x &   &   \\
       Frank (CoC+CIC)      & x & x &   &   &   &   &   &   & x &   \\
       Christine (Coq)      & x & x &   &   &   &   &   &   & x & x \\
    \hline
       cubicaltt            &   & x & x &   & x &   &   &   & x & x \\
       Agda                 & x & x & x & x & x &   &   &   & x & x \\
       Lean                 & x & x & x & x &   &   &   &   & x & x \\
...
This breakdown highlights the design trade-offs: for instance, the Groupoid spine languages start from the minimal ECC-like core of Henk (possessing only $\Pi$ and universes $U^n$), adding inductive types ($\mathrm{Ind}$) and general fixpoints ($\mathrm{Fix}$) to build Frank and Christine (CIC). In contrast, the univalent spine (Per) models MLTT-80 extended with cubical path spaces ($\Xi$), whereas Anders (HTS) unifies these features with full cubical primitives, set-theoretic universes, and higher inductive types. Contemporary systems like Agda, Lean, and NuPRL similarly occupy different positions in this design space depending on their choice of equality (strict vs. path-based) and inductive scheme representations (general inductive types vs. W-types).
...
\subsection{Frank: Minimal Inductive System \cite{Pfenning89}}\footnote{\url{https://frank.groupoid.space}}
...
\subsection{Christine: Calculus of Inductive Constructions \cite{paulinmohring}}\footnote{\url{https://christine.groupoid.space}}
Complete Calculus of Inductive Constructions with $\Sigma$, Id, and Ind.
\begin{table}[ht!]
\centering
\begin{tabular}{rl}
 cosmos :=&$\textbf{Prop}\ :\ \textbf{U}_j$ \\
    var :=&$\textbf{var}\ name$ \\
     pi :=&$\Pi\ name\ E\ E\ |\ \lambda\ name\ E\ E\ |\ E\ E$ \\
...
Semantically motivated by Coquand's Groupoid-Valued Presheaf and sheaf models \cite{coquand2021constructive} (where the Grothendieck construction models context extensions), Anders implements the CCHM cubical structure with Path, Glue, and higher inductive types (Coequ, Disc).
\begin{table}[ht!]
\centering
\begin{tabular}{rl}
 cosmos :=&$\textbf{U}_j \ |\ \textbf{V}_k$ \\
...
& \textbf{$|C_0|$} \\
\hline
0  & Henk       & Set Theory      & $\Pi$, $U^n$ & 5 \\
1  & Frank      & Set Theory      & + Ind & 8 \\
2a & Christine  & Set Theory      & + Id, Fix & 14 \\
2b & Per        & Set Theory      & + $\Sigma$, Id, W, 0, 1, 2 & 26 \\
\hline
3  & Anders     & Homotopy Theory & + Path, I, Glue, HComp, HIT & 40+ \\
4a & Felix      & Homotopy Theory & + $\flat$, $\sharp$, $\Im$, $\otimes$, Plot & 10 \\
...
--- & Mike      & Category Theory & Hom, $\otimes$, $\int$, $\prod$, $\multimap$ & 13 \\
--- & Ulrik     & Category Theory & + $\mathbb{I}^{\to}$, Ext, Tw, $\leq$ & 25 \\
\hline
\end{tabular}%
}
...
\centering
\resizebox{\textwidth}{!}{%
\begin{tabular}{|l|c|c|l|p{9cm}|}
\hline
\textbf{Functor} & \textbf{Source} & \textbf{Target} & \textbf{Status} & \textbf{Semantics \& Information Loss} \\
\hline
...
\textbf{Quantum} & Jack & Urs & Yes & Extends differential K-theory with configuration spaces, braids, and linear types. \\
\hline
\end{tabular}%
}
...
A recurring ambition in the development of higher-dimensional type theories is the pursuit of a singular,
comprehensive foundational language --- a formal system capable of expressing discrete combinatorial presentations,
directed synthetic category theory, and univalent cohesive geometry simultaneously.
However, as demonstrated by the categorical obstructions documented in this article,
we conclude that \emph{no natural unified foundation is viable}. Any attempt to internalize this entire
hierarchy into a single categorical semantics would inevitably require rigidification into strict bicategories,
tricategories, or infinitely coherent higher-categorical structures. Such a global strictification is highly
non-natural and introduces prohibitive complexity. Instead, the mathematical reality of higher-dimensional
formalisms naturally forms a \emph{functorial mesh of transports}.

The obstructions to a unified foundation are deeply rooted in the underlying geometry of the respective topoi.
For example, consider the embedding of Simplicial Type Theory (\textbf{Ulrik}) into Cubical HoTT (\textbf{Anders}).
The directed interval $\mathbb{I}^{\to}$ and its associated Segal and Rezk completeness conditions depend
fundamentally on an asymmetrical, directed path space. When attempting to interpret this within the symmetric,
invertible cubical paths of \textbf{Anders}, one is forced to invoke the \textbf{Core} functor. This functor
aggressively truncates the directed $\infty$-category, extracting only its maximal $\infty$-groupoid of isomorphisms.
The directed geometric structure is irreversibly collapsed by this essential geometric morphism.

Similarly, the transition from discrete algebraic presentations (\textbf{Dan}) to synthetic homotopy
theories cannot be accomplished via internal deformation. The combinatorial algebra of face maps,
degeneracy maps, and strict boundary operators resides in a category devoid of continuous path spaces
or univalent universes. To interpret these discrete generators within the synthetic structures of \textbf{Mike}
or \textbf{Ulrik}, one must employ the \textbf{Lift} functor to fully realize them as categorical objects.
There exists no continuous adjunction from this rigid, discrete combinatorial layer into the cohesive
$\infty$-topoi of \textbf{Felix} or the linear configuration spaces of \textbf{Urs}.

Ultimately, the architecture of modern higher-dimensional mathematics is irreducibly pluralistic.
The ``Groupoid Infinity'' stack acknowledges this by treating specialized theories not as fragments
of a missing monolith, but as distinct objects in a category of formal languages. By relying on a
functorial mesh of geometric morphisms, discrete embeddings, and modal coreflections, we achieve
a robust and mathematically rigorous ecosystem whose compositional power far exceeds that of any
artificially strictified unified theory.
...
\newblock In S.~Abramsky, Dov~M. Gabbay, and S.~E. Maibaum, editors, {
  \em Handbook of Logic in Computer Science (Vol. 2)}, pages 117--309,
  New York, NY, USA, 1992. Oxford University Press, Inc.

\bibitem[BMSS11]{birkedal2011first}
Lars Birkedal, Rasmus~Ejlers Mogelberg, Jan Schwinghammer, and Kristian Stovring.
\newblock First steps in synthetic guarded domain theory: step-indexing in the topos of trees.
\newblock In {\em 2011 IEEE 26th Annual Symposium on Logic in Computer Science}, pages 55--64. IEEE, 2011.
...
\bibitem[CHS26]{Coq88}
Thierry Coquand, Jonas H{\"{o}}fer, and Christian Sattler.
\newblock Constructive higher sheaf models with applications to synthetic mathematics.
\newblock Extended version, \url{https://arxiv.org/pdf/2605.15126}, 2026.

\bibitem[CRS21]{coquand2021constructive}
Thierry Coquand, Fabian Ruch, and Christian Sattler.
\newblock Constructive sheaf models of type theory.
\newblock {\em Mathematical Structures in Computer Science}, 31(9):979--1002, 2021.
