...
\begin{abstract}
In this article, we investigate the conjecture that no single formal language can serve as a universal foundation for the entirety of mathematics, proposing instead that mathematics is best served by a system of specialized languages connected by functorial transports. We analyze the Groupoid Infinity stack of formal languages as a \emph{lattice} of internal languages, rather than a linear tower. The languages fall into three distinct branches: a \emph{constructive type theory and set analysis} spine (Henk--Frank--Christine), a \emph{super homotopy} spine extending to differential cohesion and quantum spectra (Henk--Per--Anders--Urs), and an \emph{infinity category theory} spine (Dan--Mike--Ulrik). Because their semantic and syntactic structures are fundamentally incompatible, they cannot be unified into a single abstract syntax tree (AST). We demonstrate how these separate domains interact functorially---for instance, simplicial type theory (Ulrik/STT) provides the directed-homotopy backbone for Urs's 3-layer architecture. Finally, we construct syntactic CW-complexes and chain complexes for each language individually, define their cohomology groups, and assemble a spectral sequence over the full lattice of language categories, providing a rigorous mathematical framework for this multi-language foundation.
\end{abstract}
...
\begin{tabular}{lll}
\hline
\textbf{Language / Primitive} & \textbf{Semantic Category / Model} & \textbf{Application \& Physical/Mathematical Domain} \\
\hline
...
Per ($O_W$) & $\mathbf{LCCC} + \mathbf{W}$ & Martin-L\"{o}f Type Theory MLTT-80 (inductive W-types) \\
...
& \mathbf{Henk} \ar[d, "\text{+$\Sigma$,Id,W}"] \ar[dl, bend right=15, "\text{extract}"'] \ar[dr, bend left=15, "\text{extract}"] & \\
\mathbf{Frank} \ar[d, "\text{+$\Sigma$,Id}"'] & \mathbf{Per} \ar[d] \ar[l, "\text{+Ind}"] \ar[r, bend left=15, "\text{extract}"] & \mathbf{Dan} \ar[d, dashed, "\text{Lift}"] \\
...
\begin{itemize}
    \item \textbf{Structural Core:} At the base, Locally Cartesian Closed Categories ($\mathbf{LCCC}$) provide the semantics for $\mathbf{MLTT}$, the pure structural type theory. Upgrading the semantics to $\infty$-categories adds globular path equality, yielding Cubical Type Theory with Path types but without Higher Inductive Types (HITs). Finally, a full $\infty$-topos models Cubical Type Theory with HITs, which maintains a very lightweight core and syntax.
    \item \textbf{Cohesive Modalities:} Cohesive Type Theory (Felix) is developed specifically to model modalities such as connectedness, compactness, and infinitesimal shapes. Differential Cohesive Type Theory acts as the internal language for Differential Cohesive $\infty$-topoi, adding extra structure to $\infty$-topoi just as $\infty$-categories add path equality to LCCCs. These are typically modeled in the base library via undefined axioms (see \texttt{cohesivett}).
    ...
    \item \textbf{Directed and Simplicial Topoi:} Moving toward spatial and temporal operational algebras, presheaf categories ($\mathbf{Pres}$) provide the semantics for Guarded Type Theory (Dan). Incorporating strict directional structures leads to $\mathbf{1\text{-}Cat}$, acting as the semantics for Directed Type Theory (Mike). Finally, scaling to higher dimensions, $\mathbf{(\infty, 1)\text{-}Cat}$ serves as the model for Simplicial Type Theory (Ulrik / Riehl-Shulman STT), extending directed paths with a Segal-like directed interval.
\end{itemize}
...
\newpage
\subsection{Anders: Cubical HoTT}
CCHM cubical structure with Path, Glue, and
higher inductive types (Coequ, Disc).
\begin{table}[ht!]
\centering
\begin{tabular}{rl}
 cosmos :=&$\textbf{U}_j \ |\ \textbf{V}_k$ \\
...
\centering
\begin{tabular}{rl}
     cat :=&$\textbf{var}\ name\ |\ \textbf{op}\ C\ |\ C\ \times\ C$ \\
    type :=&$\textbf{Hom}_C\ E\ E\ |\ E\ \otimes\ E\ |\ \int^{C}\ name\ E\ |\ \int_{C}\ name\ E$ \\
         &$E\ \multimap\ E\ |\ name\ \textrm{args}$ \\
    term :=&$\textbf{id}\ E\ |\ \textbf{id}_J\ E\ |\ \textbf{id}_{J,cov}\ E\ |\ \textbf{id}_{J,contra}\ E$ \\
         &$E\ \otimes\ E\ |\ \textbf{let}\ name\ \otimes\ name\ \mathbf{=}\ E\ \textbf{in}\ E$ \\
         &$\mathbf{coend\_in}\ name\ name\ name\ E\ |\ \textbf{let}\ \mathbf{coend\_in}\ name\ name\ \mathbf{=}\ E\ \textbf{in}\ E$ \\
         &$\mathbf{end\_in}\ name\ E\ |\ \textbf{let}\ \mathbf{end\_in}\ name\ name\ name\ \mathbf{=}\ E\ \textbf{in}\ E$ \\
         &$\lambda\ name\ E\ |\ E\ E\ |\ \textbf{var}\ name$ \\
\end{tabular}
\end{table}
...
\begin{tabular}{rl}
 ...
 modalpi :=&$\Pi\ \textrm{shape}\ E\ E\ |\ \lambda\ \textrm{shape}\ E\ |\ E\ E$ \\
     tw :=&$\textbf{Tw}\ E\ |\ \textbf{tw\_p0}\ E\ |\ \textbf{tw\_p1}\ E$ \\
