...
branches: a \emph{constructive type theory} spine (Henk--Frank--Christine/Per--Anders--Urs),
an \emph{operational algebra} branch (Dan), and a \emph{directed category theory}
branch (Mike--Ulrik). These branches are connected by functorial relationships
but their syntactic term structures are fundamentally incompatible and cannot be
...
backbone for its 4-layer architecture: Homotopy, Super-Smooth, Differential
K-Theory, and Quantum Linear types. We construct syntactic CW-complexes and chain
complexes for each language individually, define their cohomology groups, and
assemble a spectral sequence over the full lattice of language categories.
\end{abstract}
...
\section{The Three Branches of the Language Lattice}
...
\mathbf{Laurent} \ar[r, "\text{Smooth}"] & \mathbf{Urs.SupSmth} \ar[d, "\text{Diff}"] & \mathbf{Ulrik} \ar[ul, dashed, "\text{Core}"'] \ar[l, red, bend left=15, "\text{STT}"] \\
& \mathbf{Urs.DiffK} \ar[d, "\text{Quantum}"] & \\
& \mathbf{Urs.Quantum} & 
\end{tikzcd}
\end{center}
...
\subsection{The Fine-Grained Primitive Lattice}
...
The categories of models for the Groupoid Infinity language stack form a planar, coherent lattice of categories radiating from the category of dependent products $\mathbf{\Pi}\text{-}\mathbf{Cat}$ (Henk), extending in four cardinal directions (dependent sums, inductive types, path objects, and bicartesian closed structures), and completing in cohesive and directed higher categories:
\begin{center}
\scriptsize
\begin{tikzcd}[row sep=large, column sep=0.35em]
...
\mathbf{Cohesive}\ \infty\text{-}\mathbf{Topos}\ \text{(Urs.SupSmth)} \ar[u, leftarrow, "\text{Modal}"'] & & \mathbf{LCCC} + \mathbf{Ind}\ \text{(Christine)} & \mathbf{\Pi}\text{-}\mathbf{Cat} + \mathbf{Ind}\ \text{(Frank)} \ar[l, dashed] & \mathbf{(\infty, 1)\text{-}Cat}\ \text{(Ulrik)} \\
\mathbf{Diff}\ \infty\text{-}\mathbf{Topos}\ \text{(Urs.DiffK)} \ar[u, leftarrow, "\text{Diff}"'] & & & & \\
\mathbf{Quantum}\ \infty\text{-}\mathbf{Topos}\ \text{(Urs.Quantum)} \ar[u, leftarrow, "\text{Quantum}"'] & & & &
\end{tikzcd}
\end{center}
\end{theorem}

\newpage
\section{Branch I: Inductive Type Theory and Analysis}
...
\newpage
\section{Branch II: Homotopical and Cohesive Spine}
...
Urs is not a single monolithic type system, but rather a family of four layered sublanguages where each layer builds on top of the syntax, modalities, and constructors of the previous one:
...
\hline
\textbf{I. Urs.Homotopy} & \texttt{universe, forall, sigma, exists, real,} &
    $U^n_g$, $\Pi$, $\Sigma$, $\exists$, $\mathbb{R}$, \\
 & \texttt{path, glue, em} &
    Path, Glue, Eilenberg--MacLane \\
\hline
\textbf{II. Urs.SupSmth} & \texttt{graded, flat, sharp, bose,} &
    Bose/Fermi grades, $\flat$, $\sharp$, \\
 & \texttt{fermi, tensor, smth, supsmth} &
    $A \otimes B$, SmthSet, SupSmthSet \\
\hline
\textbf{III. Urs.DiffK} & \texttt{spectra, grpd, form,} &
    Spectrum, Susp, Grpd, $\Omega^n(X)$, \\
 & \texttt{ku, diff, diffku} &
    $KU^{\tau}_G$, $d$, DiffKU \\
\hline
\textbf{IV. Urs.Quantum} & \texttt{conf, braid, qubit, linear} &
...
\begin{remark}[The 4-Layer Architecture]
The sublanguages represent a progressive structure for formalizing mathematical and
physical concepts:
\begin{enumerate}
  \item \textbf{Urs.Homotopy}: Foundation for homotopical properties of spaces,
    path types, glue types, and Eilenberg--MacLane spaces.
  \item \textbf{Urs.SupSmth}: Graded universes and tensors, group
  ...
  \item \textbf{Urs.DiffK}: Homotopical foundations via
  ...
  \item \textbf{Urs.Quantum}: Configuration spaces and braids for quantum
    statistics, supporting quantum systems and linear types ($!A$) for
    resource-sensitive quantum programming.
\end{enumerate}
...
\subsubsection{Urs.SupSmth (Layer I)}
...
\subsubsection{Urs.DiffK (Layer II)}
...
\subsubsection{Urs.Quantum (Layer III)}
The top layer, adding configuration spaces, braids, and linear resource types ($!A$) for quantum programming.
\begin{lstlisting}[language=Caml]
type exp_quantum =
  | DiffK of exp_diff
  | Qubit of exp_quantum * exp_quantum | Linear of exp_quantum
  | AppQubit of exp_quantum * exp_quantum * exp_quantum
  | FuseQubit of exp_quantum * exp_quantum * exp_quantum
  | Config of int * exp_quantum | Braid of int * exp_quantum
\end{lstlisting}
...
\newpage
\section{Branch III: Operational and Directed Category Theory}
...
4a & Urs.Homotopy & CTT/Modal & + Eilenberg--MacLane, Reals & 7 \\
4b & Urs.SupSmth & CTT/Modal & + $\flat$, $\sharp$, $\Im$, $\otimes$, Plot & 10 \\
4c & Urs.DiffK & CTT/Modal & + Spectra, Susp, Grpd, Forms, DiffKU & 12 \\
4d & Urs.Quantum & CTT/Modal & + Qubit, Linear, Config, Braid & 7 \\
\hline
...
Note the split at $p=2$ (Christine/Per), the four sublanguages of Urs at $p=4$ (Homotopy, SupSmth, DiffK, Quantum), and the separate branch indices
...
\hline
\textbf{Modal} & Anders & Urs.SupSmth & Yes & Extends cubical HoTT with cohesive modalities ($\flat$, $\sharp$, $\Im$) and super-grades. \\
\hline
\textbf{Diff} & Urs.SupSmth & Urs.DiffK & Yes & Stable extension to spectra, suspensions, forms, and differential K-theory. \\
\hline
\textbf{Quantum} & Urs.DiffK & Urs.Quantum & Yes & Extends differential K-theory with configuration spaces, braids, and linear types. \\
\hline
\end{tabular}%
}
...
\subsubsection{Deep Dive: The Lift Functor ($\mathbf{Dan} \to \mathbf{Mike} / \mathbf{Ulrik}$)}
...
\subsubsection{Deep Dive: The Groupoid Core ($\mathbf{Ulrik} \to \mathbf{Anders}$)}
...
\mathbf{Dan} & \xrightarrow{\text{Lift}} & \mathbf{Mike} &
\xrightarrow{\text{Free}} & \mathbf{Ulrik} &
\xleftarrow[\text{Core}]{} & \mathbf{Anders} & \\
& & & & \downarrow\text{STT} & & \downarrow\text{Modal} & \\
& & & & \mathbf{Urs.Homotopy} & \xrightarrow{\text{Cohesive}} & \mathbf{Urs.SupSmth} & \\
& & & & \uparrow & & \downarrow\text{Diff} & \\
& & & & \mathbf{Urs.Quantum} & \xleftarrow{\text{Quantum}} & \mathbf{Urs.DiffK} & \\
& & & & \uparrow & & & \\
& & & & \mathbf{Christine} \simeq \mathbf{Per}
\xleftarrow{} \mathbf{Frank} \xleftarrow{} \mathbf{Henk}
\end{array}
\end{equation}
